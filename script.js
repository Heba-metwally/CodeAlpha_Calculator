...
function calculateResult() {
    try {
        ...
        if (!isFinite(answer)) throw new Error();

        historyText.innerText = expression + "=";
        resultText.innerText = String(answer);
        calculated = true;
    } catch {
...
function addOperator(operator) {
    ...
    if (operators.includes(lastChar) && resultText.innerText === "0") {
        historyText.innerText = historyText.innerText.slice(0, -1) + operator;
        return;
    }
    ...
    lastChar = historyText.innerText.slice(-1);
...
document.querySelector(".calculator").addEventListener("click", (e) => {
    ...
    if (value && !action) {
        if (value === "." && resultText.innerText.includes(".")) return;
        if (calculated || resultText.innerText === "Error") resetCalculator();
        if (resultText.innerText === "0" && value !== ".") {
    ...
    if (action === "clear") {
        if (resultText.innerText === "Error" || calculated) {
            resetCalculator();
            return;
        }
    ...
    if (action === "clear_all") resetCalculator();

    if (action === "sign") {
        if (resultText.innerText !== "0" && resultText.innerText !== "Error") {
            resultText.innerText = String(Number(resultText.innerText) * -1);
        }
    }
});
...
document.addEventListener("keydown", (e) => {
    ...
    if ((e.key >= "0" && e.key <= "9") || e.key === ".") {
        ...
        if (calculated || resultText.innerText === "Error") resetCalculator();
        if (resultText.innerText === "0" && e.key !== ".") {
